...
CARDS = {
         ...
         # negative
         4: {'name': 'Blockade', 'cost': 15, 'effect': 'Drastically reduces rep with a country and reduces their money by $' + str(30*MULTIPLIER) + '. Costs $10', 'alignment': -1},
         # neutral
         ...
         # negative
         8: {'name': 'Sabotage', 'cost': 5, 'effect': 'Discards a card(s) from a random opponent. Costs $5', 'alignment': -1},
         # negative
...
def active_effect_factory(state, owner):
    #state.events.append(FACTIONS[state.whose_turn] + "'s Factory has produced $10.")
...
def card_effect_technology_research(state, player):
...
def card_effect_blockade(state, player):
    # Drastically reduces rep with a country and reduces their money
    chosen_player = player
    if chosen_player == -1:
        state.events.append(FACTIONS[state.whose_turn] + " must choose a player to play that card against")
        return False
    state.players[chosen_player]['money'] -= (30 * state.multiplier)
    state.players[state.whose_turn]['reputation'][chosen_player] -= (20 * state.multiplier)
    state.players[chosen_player]['reputation'][state.whose_turn] -= (20 * state.multiplier)
    state.players[state.whose_turn]['money'] -= (10 * state.multiplier)
    state.events.append(FACTIONS[state.whose_turn] + " has blockaded " + FACTIONS[chosen_player] + " reducing their $ by " + str(30 * state.multiplier) + ".")
    return True
...
def card_effect_military_aid(state, player):
    chosen_player = player
    if chosen_player == -1:
        state.events.append(FACTIONS[state.whose_turn] + " must choose a player to play that card against")
        return False
...
def card_effect_international_summit(state, player):
...
def card_effect_posturing(state, player):
    state.players[state.whose_turn]['money'] += (30 * state.multiplier)
    state.players[state.whose_turn]['stability'] += (20 * state.multiplier)

    for player in state.players[state.whose_turn]['reputation']:
        state.players[state.whose_turn]['reputation'][player] -= (15 * state.multiplier)
        state.players[player]['reputation'][state.whose_turn] -= (15 * state.multiplier)
    state.events.append(FACTIONS[state.whose_turn] + " is attempting to intimidate other factions with their military, increasing their money and stability but decreasing their rep!")
    return True
        
def card_effect_nuclear_energy(state, player):
...
def card_effect_spy_satellite(state, player):
    chosen_player = player
    if chosen_player == -1:
        state.events.append(FACTIONS[state.whose_turn] + " must choose a player to play that card against")
        return False
    ...
    # take their money
    max = min(state.players[chosen_player]['money'], (100 * state.multiplier))
    loot = random.randint(0, max)
    state.players[chosen_player]['money'] -= loot
    state.players[state.whose_turn]['money'] += loot
    state.events.append(FACTIONS[state.whose_turn] + " has gathered intelligence via Spy Satellite on " + FACTIONS[chosen_player] + " allowing them to steal $" + str(loot))
    return True
...
def card_effect_double_agent(state, player):
    ...
    state.players[chosen_player]['goalScore'] -= (.1 * state.multiplier)
    state.players[state.whose_turn]['goalScore'] += (.1 * state.multiplier)
...
def card_effect_diplomacy(state, player):
    favorability = 0
    for player in state.players[state.whose_turn]['reputation']:
...
def card_effect_thief(state, player):
    ...
    state.players[random_player]['money'] -= (20 * state.multiplier)
    state.players[state.whose_turn]['money'] += (20 * state.multiplier)

    state.players[random_player]['reputation'][state.whose_turn] -= (20 * state.multiplier)
    state.players[state.whose_turn]['reputation'][random_player] -= (20 * state.multiplier)
    
    state.events.append(FACTIONS[state.whose_turn] + " has stolen $ from " + FACTIONS[random_player] + '! Their relationship has worsened..')
    return True

def card_effect_national_debt(state, player):
    state.players[state.whose_turn]['money'] += (50 * state.multiplier)
...
def card_effect_threat(state, player):
    ...
    state.players[chosen_player]['stability'] -= (30 * state.multiplier)
    state.players[state.whose_turn]['stability'] += (50 * state.multiplier)

    state.players[state.whose_turn]['money'] -= 20
    state.events.append(FACTIONS[state.whose_turn] + " has threatened " + FACTIONS[chosen_player] + " and caused national unstability.")
    return True
...
def event_ecologic_disaster(state):
    state.phase_events.append("A series of ecological crises erupt all over the globe, causing tensions to rise!")
    state.phase_events.append("The doomsday clock advances 10 minutes.")
...
def event_political_assassination(state):
    ...
    message += "The situation grows more perilous, and the doomsday clock advances 10 minutes closer to midnight."    
    state.phase_events.append(message)

def event_nuclear_standoff(state):
    state.clock['Minute'] += 15
    state.multiplier = 2

    message = "As the world teeters on the brink, the four factions begin the process of rapid nuclear militarization. \n"
    message += "Bracing for war, each faction readies its troops and puts its defense systems on high alert. \n"
    message += "The stakes are high, but bold and decisive action in times of crisis can be heavily rewarded. \n"
    message += "All aggression cards now have double the effect on reputation, stability, and money. \n"
    message += "The fate of the world hangs in the balance, the clock moves 15 minutes closer to midnight."
    state.phase_events.append(message)
...
CARD_EFFECTS = {
    ...
    3: card_effect_technology_research,
    4: card_effect_blockade,
    5: card_effect_election,
    6: card_effect_military_aid,
    7: card_effect_international_summit,
    8: card_effect_sabotage,
    9: card_effect_spy,
    10: card_effect_posturing,
    11: card_effect_nuclear_energy,
    12: card_effect_spy_satellite,
    13: card_effect_double_agent,
    14: card_effect_diplomacy,
